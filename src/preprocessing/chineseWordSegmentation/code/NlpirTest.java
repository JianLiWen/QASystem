...
public class NlpirTest {
	...
	public void run(String inputFilePath, String outputFilePath) {
		String argu = ".\\file\\ICTCLAS2019";	// 给出Data文件所在的路径，注意根据实际情况修改。
...
//		String system_charset = "UTF-8";
		...
		try {
			File comment = new File(inputFilePath);// 读取原始语料
			File wordSegmentComment = new File(outputFilePath);// 保存分词后的文本
			// 如果待输出文件的父路径不存在，则创建之
			...
			if (comment.isFile() && comment.exists()) {
				...
				int nCount = CLibrary.Instance.NLPIR_ImportUserDict(".\\file\\Dictionary\\自定义分词词典\\分词表扩展.txt",true);
...
//		String system_charset = "UTF-8";
	...
	public static void main(String[] args) throws Exception {
		...
		String inputFilePath = rootPath + "FAQ\\deleteEmptyNRepetition\\A_delEmpNRep.txt";// 读取原始评论
